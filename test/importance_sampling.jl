@testset "log_importance_weights(x::AbstractVector{<: Real})" begin
    ...
    R_log_weights = CSV.read(joinpath(@__DIR__, "data", "example_log_weights.csv"))
...
@testset "log_importance_weights(::AbstractVector{<: AbstractVector{<: Real}})" begin
    ...
    R_log_weights = CSV.read(joinpath(@__DIR__, "data", "example_log_weights_n_eff.csv"))
...
@testset "elpd(::AbstractVector{<: Real}, ::AbstractVector{<: Real}" begin
    m = CSV.read(joinpath(@__DIR__, "data", "example_loglik_matrix.csv"), header = 0)
    R_loo = CSV.read(joinpath(@__DIR__, "data", "example_loo.csv"))
...
@testset "elpd(::AbstractVector{<: AbstractVector})" begin
    a = example_loglik_array()
    R_loo = CSV.read(joinpath(@__DIR__, "data", "example_loo_n_eff.csv"))
